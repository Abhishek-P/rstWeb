...
def segment_main(user, admin, mode, **kwargs):
	...
	for seg_id in segs:
		...
		if first_seg:
			...
			cpout += '\t\t\t<div id="segend_post_tok'+str(tok_counter)+'" class="seg_end" onclick="act('+"'del:"+'tok'+str(tok_counter)+"'"+')" onmouseover="act('+"'show_seg_num:"+'segend_post_tok'+str(tok_counter)+"'"+')"  onmouseout="act('+"'hide_seg_num:"+"segend_post_tok"+str(tok_counter)+"')" +'">||</div>'
